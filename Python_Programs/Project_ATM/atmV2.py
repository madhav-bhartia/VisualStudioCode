...
def authenticate(accNum: int, pin: int) -> bool:
    with open("Project_ATM\\bank_accounts_2.csv", "r") as rf:
...
def addAcc(accNum: int, pin: int, accHolder: str, balance: float) -> str:
    with open("Project_ATM\\bank_accounts_2.csv", "a", newline='') as wf:
...
def delAcc(accNum: int, pin: int) -> str:
    ...
    with open("Project_ATM\\bank_accounts_2.csv", "w", newline='') as wf:
        writer = csv.writer(wf)
        for row in rows:
